...
- **Share / import**: `share` in the archive exports your labels stamped with a name you choose; `import` merges a mate's file. Their verdicts appear with their name in grey in the seen-before panel (click to fill in), as blue bands on the timeline, and in the archive with a `by` column. Duplicate-safe, and imports are stored separately so your own stats stay clean.
